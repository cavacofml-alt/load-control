...
// ---------------------------------------------------------------------------
// Valores iniciais só para os inputs — a aeronave/limites reais vêm da API,
// parametrizados pelo voo selecionado (ver useEffect de carregamento).
// ---------------------------------------------------------------------------
...
// peso (vertical) já seja real. Isto é replicado em `tow_cg`/`tow_mac`
// gravados como NULL na loadsheet assinada — ver PROGRESS.md.
...
function formatStd(std: string) {
  ...
  return date.toLocaleTimeString("pt-PT", { hour: "2-digit", minute: "2-digit", timeZone: "UTC" }) + "Z";
}
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <div className="h-2.5 w-full overflow-hidden rounded-full bg-slate-200 dark:bg-slate-800">
        <div className={`h-full rounded-full transition-all ${gaugeColor(ratio)}`} style={{ width: `${pct}%` }} />
      </div>
...
export default function FlightDetailPage() {
  ...
  const [envelope, setEnvelope] = useState<AircraftEnvelope | null>(null);
  ...
  function refreshLoadsheets() {
    getLoadsheetHistory(flightId)
      .then(setLoadsheets)
      .catch(() => {
        // histórico é secundário — uma falha aqui não deve bloquear o resto do dashboard
      });
  }

  // Carrega o voo e o perfil da aeronave associada. Sem isto não há limites
  // estruturais (MZFW/MTOW/MLAW) contra que desenhar os gauges.
  useEffect(() => {
    let cancelled = false;
    getFlight(flightId)
      .then(async (f) => {
        ...
        if (!f.aircraft_registration) {
          setLoadError("Este voo não tem aeronave atribuída — não é possível calcular a carga.");
          return;
        }
        const env = await getAircraftEnvelope(f.aircraft_registration);
        if (!cancelled) setEnvelope(env);
      })
      .catch((err) => {
        if (!cancelled) {
          setLoadError(err instanceof ApiError ? `(${err.status}) ${err.message}` : "Falha ao contactar a API.");
        }
      });
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    if (!flight?.aircraft_registration) return;
    const registration = flight.aircraft_registration;
    const currentRequestId = ++requestIdRef.current;

    const timer = setTimeout(async () => {
      setLoading(true);

      const pax_loads = Object.fromEntries(cabinZones.map((zone) => [zone.code, { ADULT: zone.pax }]));
  ...
  }, [flight?.aircraft_registration, cabinZones, cargoRows, takeOffFuel, tripFuel]);
  ...
  async function handleSign() {
    ...
    try {
      ...
      setSignError(err instanceof ApiError ? `(${err.status}) ${err.message}` : "Falha ao contactar a API.");
    } finally {
      setSigning(false);
    }
  }
  ...
  if (loadError) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-50 p-6 dark:bg-slate-950">
        <div className="max-w-md rounded-lg border border-red-300 bg-red-50 px-4 py-3 text-sm text-red-800 dark:border-red-900 dark:bg-red-950 dark:text-red-200">
          <strong className="font-semibold">Erro: </strong>
          {loadError}
          <div className="mt-2">
            <Link href="/" className="underline">
              ← Voltar à lista de voos
            </Link>
          </div>
        </div>
      </div>
  ...
  if (!flight || !envelope) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-50 dark:bg-slate-950">
        <p className="text-sm text-slate-400 dark:text-slate-500">A carregar voo…</p>
      </div>
    );
  }
  ...
  const zfwPoint = result ? { mac: result.mac_zfw, weight: result.zfw, label: "ZFW" } : null;
  ...
  const towWeight = result?.tow ?? envelope.dow + takeOffFuel;
  const ldwWeight = result?.ldw ?? envelope.dow + takeOffFuel - tripFuel;
  ...
  const canSign = result !== null && result.within_limits && !signing;
  const latestLoadsheet = loadsheets[0];
    ...
    <div className="flex h-screen flex-col overflow-hidden bg-slate-50 font-sans text-slate-900 dark:bg-slate-950 dark:text-slate-100">
      ...
      <header className="flex shrink-0 items-center justify-between border-b border-slate-200 bg-white px-6 py-3 dark:border-slate-800 dark:bg-slate-900">
        <div className="flex items-center gap-6">
          <Link href="/" className="text-xs text-slate-400 hover:underline dark:text-slate-500">
            ← Voos
          </Link>
          ...
          <div>
            ...
            <div className="text-lg font-bold tracking-tight">{flight.aircraft_registration}</div>
          </div>
          <div className="h-8 w-px bg-slate-200 dark:bg-slate-800" />
          <div>
            <div className="text-[11px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">
            ...
            <div className="text-lg font-bold tracking-tight">{formatStd(flight.std)}</div>
          </div>
        </div>
      ...
      <main className="grid min-h-0 flex-1 grid-cols-12 gap-4 overflow-hidden p-4">
              ...
              {cabinZones.map((zone) => (
                <div key={zone.code} className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <div className="text-[11px] text-slate-400 dark:text-slate-500">Capacidade {zone.capacity}</div>
                  </div>
                  <input
                    type="number"
                    min={0}
        ...
        {/* Centro: Weight Cascade + Assinatura */}
        <div className="col-span-4 flex min-h-0 flex-col gap-4 overflow-y-auto pr-1">
          <SectionCard title="Weight Cascade">
            <div className="flex flex-col gap-5 py-1">
              <div className="flex flex-col gap-1">
                ...
                <span className="text-2xl font-bold tracking-tight">{formatKg(envelope.dow)}</span>
                <span className="text-[11px] text-slate-400 dark:text-slate-500">DOI {envelope.doi.toFixed(2)}</span>
              </div>

              <div className="flex justify-center text-slate-300 dark:text-slate-700">↓</div>

              <WeightGauge label="ZFW" actual={result?.zfw ?? envelope.dow} limit={envelope.mzfw} />

              <div className="flex justify-center text-slate-300 dark:text-slate-700">↓</div>

              <WeightGauge label="TOW" actual={towWeight} limit={envelope.mtow} />

              <div className="flex justify-center text-slate-300 dark:text-slate-700">↓</div>

              <WeightGauge label="LDW" actual={ldwWeight} limit={envelope.mlaw} />
          ...
          <SectionCard title="Assinatura de Loadsheet">
            <div className="flex flex-col gap-3">
              <div className="rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-[11px] text-amber-800 dark:border-amber-900 dark:bg-amber-950 dark:text-amber-200">
                <strong className="font-semibold">Demonstração:</strong> não existe autenticação real — a assinatura
                usa uma identidade fixa (DEMO_SIGNER_PROFILE_ID). O TOW CG/%MAC não é calculado (só o ZFW é real) —
                esta não é uma loadsheet certificada.
              </div>
              ...
              <button
                ...
                disabled={!canSign}
                onClick={handleSign}
                className="rounded-md bg-slate-900 px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-slate-700 disabled:cursor-not-allowed disabled:bg-slate-300 dark:bg-slate-100 dark:text-slate-900 dark:disabled:bg-slate-700"
              >
                {signing ? "A assinar…" : "Finalizar & Assinar"}
              </button>
              {result !== null && !result.within_limits && (
                <span className="text-[11px] text-red-600 dark:text-red-400">
                  Não é possível assinar: um ou mais pesos excedem os limites estruturais.
                </span>
              )}

              {latestLoadsheet && (
                <div className="mt-2 flex flex-col gap-1 border-t border-slate-100 pt-2 text-[11px] text-slate-500 dark:border-slate-800 dark:text-slate-400">
                  <span className="font-semibold text-slate-600 dark:text-slate-300">
                    Histórico ({loadsheets.length} versão{loadsheets.length !== 1 ? "ões" : ""})
                  </span>
                  {loadsheets.map((ls) => (
                    <span key={ls.id}>
                      v{ls.version} · {ls.document_type} · ZFW {formatKg(ls.zfw)} · TOW {formatKg(ls.tow)} ·{" "}
                      {new Date(ls.signed_at).toLocaleString("pt-PT")}
                    </span>
                  ))}
                </div>
              )}
            ...
            <div className="h-80 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart margin={{ top: 10, right: 20, bottom: 20, left: 10 }}>
                  ...
                  />
                  <Tooltip
                    formatter={(value, name) => (name === "weight" && typeof value === "number" ? formatKg(value) : value)}
                  />
                  <Line
