...
## 2026-08-29 — Lista de voos + assinatura de loadsheet (demo)

- **Refactor DRY primeiro**: extraí a lógica de `/calculate` para `services/calculation_service.py::run_calculation()` — devolve um `CalculationResult` (zfw/tow/ldw/lizfw/cg_zfw/mac_zfw/limites, mais `tow_cg`/`tow_mac` sempre `None`, ver abaixo). Tanto `/api/v1/load-control/calculate` como o novo `/api/v1/loadsheets` chamam esta função — não há dois sítios a fazer a mesma matemática correndo o risco de divergir. A validação `trip_fuel > take_off_fuel` mudou-se para dentro do serviço; a ordem de validação (fuel check vs. registration 404) mudou ligeiramente mas nenhum teste dependia da ordem antiga.
- **Novo endpoint `GET /api/v1/aircraft/{registration}`**: devolve o `AircraftEnvelope` (MZFW/MTOW/MLAW/DOW/DOI etc.). Não fazia parte do pedido original, mas sem ele o dashboard não tinha como saber os limites estruturais de uma aeronave diferente do TC-JNH hardcoded — necessário para o dashboard deixar de estar preso a um único voo/aeronave.
- **`GET /api/v1/flights`, `GET /api/v1/flights/{id}`**: `flight_repository.py` usa o embedding do PostgREST (`select("*, aircraft(registration)")`) para trazer a matrícula da aeronave numa só query.
- **`GET /api/v1/loadsheets/{flight_id}` (histórico) e `POST /api/v1/loadsheets` (assinar)**: `loadsheet_repository.create_loadsheet` lê a última versão do voo e insere a seguinte (`supersedes_id` a apontar para a anterior) — nunca UPDATE, a tabela é append-only por trigger na BD.
- **Migração `20260829000000_nullable_tow_cg_mac.sql`**: `loadsheets.tow_cg`/`tow_mac` passam a aceitar `NULL`, com `COMMENT ON COLUMN` a explicar porquê. **Não consegui aplicá-la eu próprio** — não há Supabase CLI autenticado neste ambiente (`supabase login` nunca foi corrido) nem a password direta da BD no `.env`, só a `SUPABASE_SERVICE_ROLE_KEY` (API REST/Auth, não DDL). Pedi ao utilizador para a correr no SQL editor do dashboard Supabase.
- **Frontend**: `app/page.tsx` deixou de ser o dashboard — passa a ser a lista de voos (`GET /api/v1/flights`). O dashboard antigo mudou-se para `app/flights/[id]/page.tsx`, agora parametrizado pelo voo/aeronave reais (`getFlight` + `getAircraftEnvelope`) em vez das constantes `FLIGHT`/`AIRCRAFT` fixas no TC-JNH. Novo painel "Assinatura de Loadsheet" com o botão "Finalizar & Assinar" (desativado quando `within_limits` é false, espelhando o hard-block do backend) e histórico de versões.
- **Seeds**: `scripts/seed_demo_signer.py` (cria `auth.users` + `profiles` de demonstração via Admin API — necessário porque `profiles.id` é FK de `auth.users(id)`) e `scripts/seed_demo_flight.py` (dois voos TK1234/TK1235 ligados ao TC-JNH). Ambos corridos contra a Supabase de produção; `DEMO_SIGNER_PROFILE_ID` gravado em `backend/.env` e `.env.example` (ainda falta configurar no Railway).
- **Testes**: `test_aircraft_envelope_api.py`, `test_flights_api.py`, `test_loadsheets_api.py` (novos, seguindo o padrão de `monkeypatch` já usado em `test_api.py`) — 58/58 a passar. `npx tsc --noEmit` e `npm run build` do frontend também limpos.

### A minha opinião no momento

- As três decisões de arquitetura que identifiquei antes de escrever código, mantidas tal como planeadas:
  1. **`DEMO_SIGNER_PROFILE_ID`** é um placeholder deliberadamente temporário e feio — o backend decide quem assina, nunca o cliente, porque sem autenticação real um `signed_by` vindo do frontend seria trivialmente falsificável. Isto tem de ser substituído por autenticação a sério antes de este sistema servir uma companhia aérea real; não é um detalhe, é a única coisa a impedir qualquer pessoa de "assinar" uma loadsheet em nome de outro operador.
  2. **`tow_cg`/`tow_mac` a `NULL`** em vez de inventar um número plausível: prefiro um buraco visível (`NULL`, aviso amarelo na UI) a um valor calculado com uma fórmula que sei estar incompleta, numa tabela que é, por desenho, imutável e de auditoria. Um número errado mas "com boa cara" numa loadsheet é pior do que a ausência óbvia de número.
  3. **Hard-block ao assinar, não-bloqueante ao pré-visualizar**: mantido — `/calculate` é uma ferramenta de trabalho, `/loadsheets` é uma garantia formal à tripulação. Fundir os dois comportamentos seria mais simples de código mas errado de propósito.
- O maior risco novo introduzido aqui não é nenhuma destas três — é o `create_loadsheet` não ser atómico (ler a última versão + inserir a seguinte, sem transação serializável). Com um único operador por voo (o caso de uso atual) não é um problema prático; passa a ser um problema real no dia em que dois utilizadores possam assinar o mesmo voo em paralelo. Documentei isto no docstring do repositório em vez de resolver agora — resolver bem exigiria uma função RPC no Postgres (transação do lado do servidor), que é mais trabalho do que este passo justifica.
- Não consegui aplicar a migração eu próprio, e acho que vale a pena ser explícito sobre porquê: este ambiente nunca fez `supabase login`, e o `.env` só tem a chave de service role (API), não a password da base de dados. As duas vias que teria para DDL direto (CLI autenticado, ou `psql` com a password) estão ambas fechadas; a via que resta é o SQL editor do dashboard, que só o utilizador consegue usar. Isto não bloqueou o resto do trabalho (o schema de `flights`/seeds não precisa da migração), mas bloqueia testar `/loadsheets` de ponta a ponta contra produção até a migração ser aplicada manualmente.

## Próximos passos possíveis (não decididos)
- [ ] Aplicar a migração `20260829000000_nullable_tow_cg_mac.sql` em produção (pendente de ação manual do utilizador no SQL editor do Supabase) e só depois verificar `/loadsheets` end-to-end contra o Railway
- [ ] Configurar `DEMO_SIGNER_PROFILE_ID` nas env vars do Railway (só está no `.env` local por agora)
- [ ] Calcular o efeito do combustível no índice/CG (Secção C do AHM565) para que os pontos de TOW/LDW no envelope deixem de depender de %MAC estimado, e `tow_cg`/`tow_mac` deixem de ser `NULL`
- [ ] Resolver a condição de corrida em `loadsheet_repository.create_loadsheet` (ler último `version` + inserir não é atómico) — provavelmente via uma função RPC transacional no Postgres
- [ ] Autenticação real de utilizadores para substituir `DEMO_SIGNER_PROFILE_ID`
